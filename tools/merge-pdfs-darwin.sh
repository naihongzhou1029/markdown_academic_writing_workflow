#!/bin/bash
# Merge PDFs on macOS using pdfunite or Ghostscript
...
if command -v pdfunite >/dev/null 2>&1; then
    ...
    echo "Neither pdfunite nor gs found. Install poppler (pdfunite) with 'brew install poppler'" >&2
    echo "or Ghostscript with 'brew install ghostscript' to enable PDF merging." >&2
    exit 1
fi
